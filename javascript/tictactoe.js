...
function boardButtonClicked(button) {
	if (playerTurn && button.disabled == false) {
		button.innerHTML = "X";
...
function makeComputerMove() {
	let button = getGameBoardButtons()[Math.floor(Math.random() * 8)];
	while (button.disabled ) {
		button = getGameBoardButtons()[Math.floor(Math.random() * 8)];
	}
